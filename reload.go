...
func sendReload() {
	...
	//message := bytes.TrimSpace([]byte(`http://localhost:8080/images/oak_debugconsole_chews_cpu.png`))
	var message []byte
	switch next % 3 {
	case 0:
		message = bytes.TrimSpace([]byte(`{ "text":"notes/code that generated this.", "image":"oak_debugconsole_chews_cpu.png" }`))
	case 1:
		message = bytes.TrimSpace([]byte(`{ "text":"just some code" }`))
	case 2:
		message = bytes.TrimSpace([]byte(`{ "image":"zt_record_depth_on.png" }`))
	}
	next++
